...
export class BookRenderService {
    ...
    private async renderMarkdown({ app, file, config, book, extra }: ParamType): Promise<DocType> {
        ...
        try {
            ...
            console.log(`Markdown render time: ${new Date().getTime() - startTime}ms`);
    ...
    // ... existing code ...
    // 保留所有现有的辅助方法：collectFileNodes, getFrontMatter, extractCssClasses, 
    // processBlockReferences, createRenderFragment, fixInternalLinks, fixWaitRender, 
    // fixCanvasToImage, getAllStyles, getPatchStyles, 等等
